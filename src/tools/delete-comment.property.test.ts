 ...
 * Property tests for delete_comment tool
 * Validates: Requirements 5.2, 5.3, 5.5, 5.7, 13.1
 */
...
describe("delete_comment — Property 1: Access check before API call", () => {
  it("drive.comments.delete is never called for denied IDs", async () => {
    await fc.assert(
      fc.asyncProperty(docIdArb, commentIdArb, async (docId, cId) => {
        const ac = makeDeniedAC(docId);
        const mock = vi.fn();
        const dc = { comments: { delete: mock } } as unknown as drive_v3.Drive;
        const result = await deleteComment({ documentId: docId, commentId: cId }, dc, ac);
...
describe("delete_comment — Property 3: Access denied error message format", () => {
  it("returns exact access denied message", async () => {
    await fc.assert(
      fc.asyncProperty(docIdArb, commentIdArb, async (docId, cId) => {
        const ac = makeDeniedAC(docId);
        const dc = { comments: { delete: vi.fn() } } as unknown as drive_v3.Drive;
        const result = await deleteComment({ documentId: docId, commentId: cId }, dc, ac);
...
describe("delete_comment — Property 6: returns commentId in confirmation", () => {
  it("returned text contains the commentId", async () => {
    await fc.assert(
      fc.asyncProperty(docIdArb, commentIdArb, async (docId, cId) => {
        const ac = makeAllowedAC(docId);
        const dc = { comments: { delete: vi.fn().mockResolvedValue({}) } } as unknown as drive_v3.Drive;
        const result = await deleteComment({ documentId: docId, commentId: cId }, dc, ac);
...
describe("delete_comment — Property 11: API errors caught", () => {
  it("returns isError: true when drive.comments.delete throws", async () => {
    await fc.assert(
      fc.asyncProperty(docIdArb, commentIdArb, async (docId, cId) => {
        const ac = makeAllowedAC(docId);
        const dc = { comments: { delete: vi.fn().mockRejectedValue(new Error("fail")) } } as unknown as drive_v3.Drive;
        const result = await deleteComment({ documentId: docId, commentId: cId }, dc, ac);
        expect(result.isError).toBe(true);
      }),
      { numRuns: 100 },
